...
function parseCookie(setCookie = []) {
  ...
  return list.filter(Boolean).map((v) => String(v).split(";")[0]).join("; ");
}
...
function isOk(status, data) {
  ...
  if (typeof data === "string") {
    ...
    if (["error", "failed", "user_retry_required"].includes(lowered)) return false;
  }
...
export async function downr(url) {
  try {
    ...
    const result = await tryDownload(input);
