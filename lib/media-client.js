...
export async function resolveFullAudioTrack(track) {
  ...
  // >>> TAMBAHAN BARU: Jika ini link Spotify, gunakan scraper Spotidown
  if (track.url.includes("spotify.com")) {
    const res = await fetch("/api/spotidown", {
      method: "POST",
    ...
    if (!json?.Status) {
      throw new Error(json?.Error || "Gagal scrape Spotify full track.");
    }

    return {
      ...track,
      // Pass melalui proxy agar tidak terkena error CORS browser
      audioUrl: `/api/media-proxy?url=${encodeURIComponent(json.Result.audioUrl)}`,
      sourceAudioUrl: json.Result.audioUrl,
      isFullAudio: true
    };
  }
  // <<< END TAMBAHAN BARU

  // Fallback untuk URL platform lain (TikTok, Instagram, dll)
