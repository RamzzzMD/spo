...
async function getAuth(force = false) {
  if (!force && session.token && Date.now() < session.expires - 60000) {
    return session;
  }
  ...
  const client = await clientRes.json().catch(() => null);

  if (!client?.granted_token?.token) {
    throw new Error("Spotify client token request failed");
  }

  session.token = token.accessToken;
  session.clientToken = client.granted_token.token;
  session.expires =
    Number(token.accessTokenExpirationTimestampMs) || now + 3000000;
...
async function discoverHash() {
  ...
  try {
    const htmlRes = await fetch("https://open.spotify.com/", {
      headers: {
        "user-agent": UA
      },
      cache: "no-store"
    ...
    const mainJsRes = await fetch(mainUrl, {
      headers: {
        "user-agent": UA,
        referer: "https://open.spotify.com/"
      },
    ...
    const candidates = [
      ...new Set(
        [...mainJs.matchAll(
          /https:\/\/open\.spotifycdn\.com\/cdn\/build\/web-player\/[\w.-]*search[\w.-]*\.js/g
        )].map((x) => x[0])
      )
    ];
    ...
    for (const url of candidates) {
      ...
      const hash =
        chunkJs.match(/"searchDesktop","query","([a-f0-9]{64})"/)?.[1] ||
        null;
...
function parseTrack(d) {
  ...
  const id = String(d.uri || "").split(":")[2] || null;
...
async function getPreview(id) {
  ...
  try {
    const htmlRes = await fetch(`https://open.spotify.com/embed/track/${id}`, {
      headers: {
        "user-agent": UA
      },
      cache: "no-store"
    ...
    if (nextData) {
      return (
        JSON.parse(nextData[1])?.props?.pageProps?.state?.data?.entity
          ?.audioPreview?.url || null
      );
    }

    return (
      html.match(/https:\/\/p\.scdn\.co\/mp3-preview\/[a-zA-Z0-9]+/)?.[0] ||
      null
    );
  } catch {
    return null;
...
async function searchData(term, limit) {
  ...
  const tryHashes = async (hashes) => {
    for (const hash of hashes) {
      ...
      if (json?.data?.searchV2) {
        return json.data.searchV2;
      }
    }
  ...
  const primary = discoveredHash
    ? [discoveredHash, ...FALLBACK_HASHES]
    : FALLBACK_HASHES;
...
export async function spotifySearch(searchTerm, limit = 5) {
  ...
  try {
    ...
    const items = (data.tracksV2?.items || [])
      .map((i) => parseTrack(i.item?.data))
      .filter(Boolean)
      .slice(0, limit);
